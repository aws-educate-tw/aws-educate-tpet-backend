    ...
    try:
        ...
        if final_campaign_id:
            with ThreadPoolExecutor() as executor:
                future_run = executor.submit(run_meta_repo.get_run, final_campaign_id, run_id)
                future_camp = executor.submit(camp_master_repo.get_campaign_by_id, final_campaign_id)
                
                run_item = future_run.result()
                camp_master_item = future_camp.result()

        if not run_item or not camp_master_item:
            logger.error("Data missing - Run: %s, Camp: %s", bool(run_item), bool(camp_master_item))
        ...
        response_data = {
            "status": "SUCCESS",
            "rsvp_status": user_item.get("rsvp_status", "PENDING") if user_item else "PENDING",
