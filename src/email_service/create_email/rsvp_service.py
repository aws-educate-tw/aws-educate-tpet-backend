...
class RSVPService:
    ...
    def import_participant(
        ...
        try:
            ...
            logger.info(
                "Calling RSVP service import_participant: run_id=%s, email=%s, campaign_id=%s",
                run_id,
                email,
                campaign_id,
            )
            ...
            logger.info(
                "Successfully imported participant: run_id=%s, participant_id=%s",
                run_id,
                (result.get("data") or {}).get("participant_id"),
            )
