...
def process_sqs_message(message: Dict[str, Any], receipt_handle: str, aws_request_id: str, error_responder: ErrorResponder) -> Dict[str, Any]:
    ...
    try:
        ...
        if not ensure_database_awake():
            logger.error("Failed to wake up Aurora database after maximum retries. Request ID: %s", aws_request_id)
        ...
        logger.error("Error processing message: %s. Request ID: %s", str(e), aws_request_id)
        return {
            "statusCode": 500,
            "body": json.dumps({
