...
from slack_util import post_incident_message
from cloudwatch_util import get_metric_chart, CloudWatchError
...
def lambda_handler(event, context):
    ...
    try:
        ...
        if "Records" not in event:
            ...
            return {
                "statusCode": 400,
                "body": json.dumps({"error": "Invalid event structure: missing 'Records'"})
            }
        ...
        for idx, record in enumerate(event["Records"]):
            try:
                ...
                try:
                    ...
                    failed_records.append({"index": idx, "error": "Invalid SNS message format"})
                    continue
                ...
                if not alarm_name or not raw_state:
                    logger.error(f"Missing required fields - AlarmName: {alarm_name}, NewStateValue: {raw_state}")
                    failed_records.append({"index": idx, "error": "Missing required alarm fields"})
                    continue
                ...
                try:
                    ...
                    failed_records.append({"index": idx, "error": "Database error", "alarm": alarm_name})
                    continue
                ...
                try:
                    ...
                    logger.warning(f"CloudWatch chart generation failed (status {e.status_code}): {e.message}")
                except Exception as e:
                    logger.warning(f"Unexpected error generating chart: {e}", exc_info=True)
                
                blocks, color = build_blocks(alarm_name, description, incident_state, None)
                
                # === CASE 1: new incident ===
                if not item or not item.get("incident_open", False):
                    if incident_state == "RESOLVED":
                        logger.info(f"Skipping RESOLVED state for non-existent incident: {alarm_name}")
                        continue
                    ...
                    try:
                        ...
                        logger.error(f"Slack API error posting message for {alarm_name}: {e.response['error']}", exc_info=True)
                        failed_records.append({"index": idx, "error": "Slack API error", "alarm": alarm_name})
                        continue
                    except Exception as e:
                        logger.error(f"Failed to post Slack message for {alarm_name}: {e}", exc_info=True)
                        failed_records.append({"index": idx, "error": "Slack posting failed", "alarm": alarm_name})
                        continue
                    ...
                    try:
                        ...
                        logger.error(f"Failed to create incident in DynamoDB for {alarm_name}: {e}", exc_info=True)
                        # Incident was posted to Slack but not saved to DB - log as warning
                        logger.warning(f"Incident {alarm_name} posted to Slack but not saved to DynamoDB")
                        failed_records.append({"index": idx, "error": "Database write failed", "alarm": alarm_name})
                ...
                try:
                    ...
                    logger.info(f"Updated Slack message for {alarm_name} to state {incident_state}")
                except SlackApiError as e:
                    ...
                    logger.error(f"Failed to update Slack message for {alarm_name}: {e}", exc_info=True)
                    # Continue to update DynamoDB even if Slack update fails
                ...
                try:
                    ...
                    logger.error(f"Failed to update incident in DynamoDB for {alarm_name}: {e}", exc_info=True)
                    failed_records.append({"index": idx, "error": "Database update failed", "alarm": alarm_name})
                
            except Exception as e:
                logger.error(f"Unexpected error processing record {idx}: {e}", exc_info=True)
        ...
        if not failed_records:
            ...
            logger.warning(f"Partial success: {total_records - len(failed_records)}/{total_records} records processed")
            return {
                "statusCode": 207,
                "body": json.dumps({
